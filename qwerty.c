...
typedef struct {
    char username[30];
    char password[256]; // Adjusted to store hashed passwords
    char name[50];
    char email[50];
} User;
...
void registerUser() {
    ...
    char salt[] = "$6$xxxxxxxxxx"; // Using SHA-512
    ...
    strcpy(users[userCount].password, hashed_password);
...
void loginUser() {
    ...
    for (int i = 0; i < userCount; i++) {
        if (strcmp(users[i].username, username) == 0) {
            char* hashed_password = crypt(password, users[i].password);
